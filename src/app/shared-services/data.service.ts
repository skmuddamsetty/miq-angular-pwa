...
export class DataService {
  ...
  constructor(
    ...
    private store: Store<{
      miqList: { miqList: IQ[]; miqMap: any; selectedKey: string };
    }>
  ) {
  ...
  loadQuestionsFromDB(key: string) {
    this.storeSubscription = this.store.select('miqList').subscribe(res => {
      console.log(`inside subscribe and select key is ${res.selectedKey}`);
      key = res.selectedKey;
      if (res.miqMap && res.miqMap.get(key) === undefined) {
        let onlineDataLength;
        this.idbService
          .getAllDataForKeyInTarget(
